...
void plot_hungerford_spectrum(const char* Variable = "e", double ANorm = 0.05, const char* Option = "") {
  ...
  if (var == "e") {
    ...
    ep_hgf = new TF1("ep_hgf",GetEjectedProtontWeight,0,100,2);
    ep_hgf->SetParameter(0,0);
    ep_hgf->SetParameter(1,1);
    ep_hgf->SetParameter(1,ANorm/ep_hgf->Integral(0,100));
    ep_hgf->SetNpx(1000);

    ep_hgf->SetTitle("Ejected proton energy");
    ep_hgf->GetXaxis()->SetTitle("E, MeV");

  }
  else if (var == "p") {

    ep_hgf = new TF1("ep_hgf",GetEjectedProtontWeight,0,1000,2);
    ep_hgf->SetParameter(0,1);
    ep_hgf->SetParameter(1,1);
    ep_hgf->SetParameter(1,ANorm/ep_hgf->Integral(0,1000));
    ep_hgf->SetNpx(1000);

    ep_hgf->SetTitle("Ejected proton momentum");
    ep_hgf->GetXaxis()->SetTitle("P, MeV/c");
  }
